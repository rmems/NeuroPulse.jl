...
function _build_summary(rows, agg, seps, figures, clean::Condition, noisy::Condition)
    ...
    println(io, "| --- | --- |")
    println(io, "| `figure.png` | Mean focus retention ± 1 SD over seeds: how much temporal garbage can the signal survive before focus flips? |")
...
function main()
    ...
    println()
    for p in (config_path, metrics_path, summary_path, figures..., provenance_path)
